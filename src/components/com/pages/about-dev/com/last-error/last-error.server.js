...
function Indicators( props ){
  ...
  const maxResponseTime = props.log.reduce( (tot, cur) => (tot < cur.responseTime ) ? cur.responseTime : tot , 0 )
  ...
  return (
    <Frame>
      <Stack direction='row' spacing={2}>
        ...
        <span><Indicator title={"max response time"} value={maxResponseTime} format={"0a"} suffix={"days"} width={150}/></span>
      </Stack>
    </Frame>
  )
}
...
function ResponseTime( props ){
  ...
  return (
    <Frame>
      <Stack>
        <SubTitle>Response time</SubTitle>
        <ResponseTimeChart data={log} height={250}/>
      </Stack>
    </Frame>
  )
}
