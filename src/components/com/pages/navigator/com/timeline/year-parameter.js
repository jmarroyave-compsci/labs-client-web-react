...
function YearsParameters(props){
    ...
    const decades = useMemo( ()=> getDecades({ascending : true, all: true, unknown: true}), [])
    const current = context.state.parameters.decade
    const onClick = ( p ) => context.dispatch( { type: "CHANGE_DECADE", payload: p }) 
    const textMap = (d, idx) => <span style={{ fontSize: (d.slice(2,4) == "00") ? "110%" : '100%' }}>{(d == "?" || current == d || d.slice(2,4) == "00" || idx == 0) ? d : d.slice(2,4)}</span>
