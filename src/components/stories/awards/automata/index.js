...
export const fetchData = createAsyncThunk(`${MODEL_NAME}/fetchData`,
  async ( params, thunkAPI ) => {
    ...
    var resp = await data.fetchData( params.entity, params.year, params.page );
    return resp;
  }
)
...
const slice = createSlice({
  ...
  reducers: {
    setParams : (state, action) => {
      ...
      Router.push(`${config.page.url( state.params )}?year=${state.params.year}&page=${state.params.page}`, null, { shallow: true })
    }
  },
  extraReducers: {
